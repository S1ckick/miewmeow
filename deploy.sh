...
cd cmake-3.21.3
./bootstrap --prefix=`realpath $PWD/../env/`
make
make install
cd ..
rm -r cmake-3.21.3
...
cd swig-4.0.2
./configure --prefix=`realpath $PWD/../env/` --without-pcre
make
make install
cd ..
rm -r swig-4.0.2
...
mkdir ob-build
cd ob-build
cmake ../openbabel-openbabel-3-1-1 -DPYTHON_BINDINGS=ON -DRUN_SWIG=ON -DCMAKE_INSTALL_PREFIX=`realpath $PWD/../env/`
make -j$(nproc --all) install
cd ../
rm -r ob-build
rm -r openbabel-openbabel-3-1-1
